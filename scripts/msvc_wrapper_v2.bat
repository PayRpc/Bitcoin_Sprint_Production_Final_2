...
rem Filter all linker flags
echo.%~1 | findstr /b /C:"-Wl" > nul 2>&1
if %errorlevel% equ 0 goto filter_flag

rem Filter all -g, -f, -Q, -O flags
echo.%~1 | findstr /b /C:"-g" > nul 2>&1
if %errorlevel% equ 0 goto filter_flag

echo.%~1 | findstr /b /C:"-f" > nul 2>&1
if %errorlevel% equ 0 goto filter_flag

echo.%~1 | findstr /b /C:"-Q" > nul 2>&1
if %errorlevel% equ 0 goto filter_flag

echo.%~1 | findstr /b /C:"-O" > nul 2>&1
if %errorlevel% equ 0 goto filter_flag
...
rem Check if it's likely a source file or known flag before passing through
if "%~1:~0,1%"=="-" (
    echo [msvc_wrapper] Filtering unknown flag: %~1 >> C:\temp\msvc_wrapper_log.txt
) else (
    echo [msvc_wrapper] Unknown argument, passing through: %~1 >> C:\temp\msvc_wrapper_log.txt
    set "MSVC_ARGS=!MSVC_ARGS! %~1"
)
goto next_arg
